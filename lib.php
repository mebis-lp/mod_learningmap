...
function learningmap_cm_info_view(cm_info $cm) : void {
    ...
    if ($cm->showdescription == 1) {
        ...
        /*$PAGE->requires->js_call_amd('mod_learningmap/manual-completion-watch', 'init',
            ['coursemodules' => learningmap_get_place_cm($cm), 'mapid' => $cm->id]);*/
        $PAGE->requires->js_call_amd('mod_learningmap/completionwatcherinitializer',
            'init', ['mapId' => $cm->id, 'dependingModuleIds' => learningmap_get_place_cm($cm)]);
